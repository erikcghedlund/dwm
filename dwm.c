...
focusstack(const Arg *arg)
{
	...
	if (!selmon->sel || (selmon->sel->isfullscreen && lockfullscreen))
		return;
	if (arg->i > 0) {
		for (c = selmon->sel->next; c && (!ISVISIBLE(c, selmon) || !c->canfocus); c = c->next);
		if (!c)
			for (c = selmon->cl->clients; c && (!ISVISIBLE(c, selmon) || !c->canfocus); c = c->next);
	} else {
		for (i = selmon->cl->clients; i != selmon->sel; i = i->next)
			if (ISVISIBLE(i, selmon) && i->canfocus)
				c = i;
		if (!c)
			for (; i; i = i->next)
				if (ISVISIBLE(i, selmon) && i->canfocus)
...
tile(Monitor *m) {
    ...
    mw = m->ww - m->gappx;
    for (i = 0, my = ty = m->gappx, c = nexttiled(m->cl->clients, m); c;
